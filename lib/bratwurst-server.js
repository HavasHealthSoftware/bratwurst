...
function startServer(bratwurstServer) {
    ...
    connectApp.use('/signalr', proxy(url.parse(bratwurstServer.options.apiUrl + '/signalr')));
    connectApp.use('/api', proxy(url.parse(bratwurstServer.options.apiUrl)));

	// Set up EJS Connect middleware for each environment
			...
			if (pageData) {
				...
				fs.exists(fullPath, function(exists) {
					...
					if (exists) {
						...
						winston.debug('pageData: ', pageData);
						winston.debug('globalData: ', globalData);
						...
						try {
							...
							winston.error('error rendering: ', relativePath);
						}
